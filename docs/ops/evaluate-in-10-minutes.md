...
2. watch transport health move to `connected_no_data` or `ingesting`
...
If the second doctor run reports `summary.transport_observations[].state == "historical_only"`, MEL is telling you that this transport has prior packets stored locally without over-claiming that it is live right now.
