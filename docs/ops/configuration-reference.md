...
| `allow_remote` | bool | `false` | Allow non-loopback connections. Requires `auth` to be configured. |
...
| `data_dir` | string | `"data"` | Directory for auxiliary data files (must be writable). |
...
| `basic_auth` | object | `null` | `{"username": "...", "password": "..."}` for UI/API authentication. |
...
| `serial_device` | string | `""` | Path for serial devices (e.g. `/dev/ttyUSB0`). |
...
- `read_timeout_sec`: `15`
- `write_timeout_sec`: `5`
- `max_timeouts`: `3` (consecutive failures before transport error state)
...
| `mode` | string | `"guarded_auto"` | `observe_only`, `guarded_auto`, `approval_required`. |
| `require_approval_for_action_types` | []string | `[]` | List of action types requiring manual approval. |
| `require_approval_for_high_blast_radius` | bool | `false` | Gate actions with `mesh` or `global` impact. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Enable health scoring and anomaly detection. |
| `retention.health_snapshot_days` | int | `7` | Retention for health history snapshots. |
| `alerts.minimum_state_duration_seconds` | int | `30` | Seconds before a degraded state triggers an alert. |
...
- `configured_offline`: Reachability check failed, in retry backoff.
- `connected_no_ingest`: Link established, but no packets received yet.
- `ingesting`: Active data flow confirmed and written to SQLite.
- `historical_only`: No live connection, but historical data is available.
- `error`: Terminal failure; operator intervention likely required.
