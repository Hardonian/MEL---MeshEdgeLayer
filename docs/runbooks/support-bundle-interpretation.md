...
| `bundle.json` | Full monolith (all sections, machine-readable) | Tooling ingestion; grep for specific fields |
...
| `accepted_with_caveats` | Structurally valid but with warnings (e.g., not cryptographically verified, partial observation) |
...
3. **Transport liveness at export time.** The bundle captures database state, not real-time transport status. Check the status snapshot for transport state at export time.
4. **Causal ordering across instances.** Timeline order is instance-local. Cross-instance ordering is best-effort.
