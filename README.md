...
- **CLI:** Prefer `mel action approve|reject` (service path: audit_log, timeline, queue) over `mel control approve|reject`. The `mel control` approve/reject commands are **legacy break-glass**: they require `--i-understand-bypasses-audit`, print explicit warnings on stderr, and still perform direct SQL without canonical audit parity.
