...
func (d *DB) InsertMessage(m map[string]any) (bool, error) {
	...
	sql := fmt.Sprintf(`INSERT OR IGNORE INTO messages(transport_name,packet_id,dedupe_hash,channel_id,gateway_id,from_node,to_node,portnum,payload_text,payload_json,raw_hex,rx_time,hop_limit,relay_node) VALUES('%s',%d,'%s','%s','%s',%d,%d,%d,'%s','%s','%s','%s',%d,%d); SELECT changes() AS changes;`,
	...
	changes, err := d.Scalar(sql)
	if err != nil {
		return false, err
	}
	return asInt(changes) > 0, nil
}
