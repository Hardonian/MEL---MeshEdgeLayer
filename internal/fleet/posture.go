...
const (
	...
	// FederationEvidencePlaneUnsupported means remote-ingest federation is not implemented as a core capability.
...
func DefaultCapabilityPosture() CapabilityPosture {
	return CapabilityPosture{
		...
		FederationReadOnlyEvidenceIngest: FederationEvidencePlaneUnsupported,
		CrossInstanceActionExecution:     ActionExecutionPlaneLocalOnly,
		FleetAggregationSupported:        false,
		Notes: "Core MEL does not ship a federated evidence plane or cross-instance action execution. " +
			"Exports and operator workflows are instance-scoped; multi-instance reasoning requires explicit merge outside core or future transport.",
	}
}
